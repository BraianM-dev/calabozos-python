# Recursos visuales

- `pip-states-v3.svg`: hoja vectorial de seis estados originales de Pip (atento, parpadeo, pensando, explicando, celebrando y advertencia). Se integra como sprite CSS y no imita personajes existentes.
- `hero-evolution/*.svg`: doce retratos vectoriales originales que conservan la identidad de las cuatro clases en sus etapas inicial, veterana y legendaria.
- `hero-sprites/*.svg`: doce hojas vectoriales de seis fotogramas cada una para los estados del personaje. El primer fotograma funciona como respaldo cuando está activado **Reducir movimiento**.
- `heroes-pdf-data.js`: copia técnica embebida de los doce retratos para que la ficha PDF funcione sin conexión.
- `logo-pdf-data.js`: emblema embebido utilizado por el generador PDF local.
- `vendor/`: copias locales de CodeMirror y Skulpt para no depender de servicios externos durante la clase.
- `journey.css` y `journey.js`: recorrido inferior que representa el avance real del héroe por las misiones.
- `responsive.css`: adaptación compartida para teléfonos, tablets, pantallas pequeñas e iframes.
- `campaigns.js` y `campaigns.css`: configuración narrativa y visual de Calabozos, Jujutsu y Multiverso sobre un único motor educativo.

Los recursos SVG utilizados por la interfaz son originales del proyecto y reducen el peso de GitHub Pages. Las imágenes raster históricas pueden conservarse en el paquete de desarrollo, pero no son necesarias para ejecutar la versión web.

Estos recursos originales se distribuyen como parte de **Aventuras & Python** bajo la licencia Creative Commons Atribución 4.0 Internacional indicada en `../LICENSE`. Los nombres o referencias de terceros se rigen por los derechos de sus respectivos titulares.
